...
4. **Search for upcoming events** in two passes:
   - **Netherlands pass.** Use `WebSearch` and/or `WebFetch` against:
     - Aggregators: techmeetups.nl, dutchitchannel.nl, techleap.nl events page, eventbrite.com (Netherlands tech category), meetup.com (Amsterdam/Rotterdam/Utrecht/Eindhoven/The Hague tech), 10times.com (NL), dev.events (NL).
   ...
   - **Vendor-summit pass (both regions).** Conference aggregators **do not list cloud / data / dev-tool vendor summits**, so query the vendors' own event pages directly. For each major vendor below, search for the named regional stop and verify the date on the vendor site:
     - **Cloud / infra:** AWS Summit (Amsterdam, London, Berlin, Paris, Stockholm, Madrid, Milan), AWS re:Inforce / re:Invent Europe spin-offs, Google Cloud AI Live / Summit (Benelux, DACH, UK, France, Iberia, Italy, Nordics), Microsoft AI Tour / Ignite spotlights / Build After Hours, IBM TechXchange Europe, Oracle CloudWorld Tour, Red Hat Summit Connect (Amsterdam / Brussels / Paris / Munich / London / Milan / Madrid).
     - **Data / AI / DevOps:** Snowflake World Tour (Amsterdam, London, Paris, Munich, Stockholm, Madrid, Milan), Databricks Data + AI World Tour, MongoDB.local Europe, Confluent Current / Kafka Summit Europe, Elastic{ON} Tour, HashiConf Europe, GitLab Connect, GitHub Universe Recap (EMEA cities), Docker Build Day Europe, JFrog swampUP Europe.
     - **SaaS / business platforms:** Salesforce World Tour (Amsterdam, London, Paris, Madrid, Milan, Stockholm), ServiceNow World Forum, SAP Sapphire Europe, Workday Rising Europe, Atlassian Team Tour, Adobe Summit EMEA, HubSpot Inbound spotlights.
     - **Security:** Cisco Live EMEA, Fortinet Accelerate Europe, Palo Alto Networks Ignite EMEA, CrowdStrike Fal.Con Europe, SentinelOne OneCon EMEA, RSA Conference satellite events.
     - Vendor summits are usually **free or low-cost, one-day, AI/cloud-heavy, hosted at RAI / ExCeL / Carrousel du Louvre / Messe**. They are exactly the kind of "missing" event a casual reader notices first, so do not skip this pass even when it feels repetitive. If a vendor has not announced 2026 dates yet, do not invent one &mdash; record nothing.
